  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-80px)] overflow-hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-24 md:pb-0">
          {filteredProducts.map((product) => (
            <div key={product.id} 
              ...
              className="bg-white p-4 rounded-xl shadow-sm hover:shadow-md transition-shadow cursor-pointer border border-gray-100 flex flex-col justify-between"
            >
              <div>
              ...
              <div>
                <div className="flex justify-between items-end">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${product.stock < 50 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
      ...
      <div className="w-full md:w-96 bg-white border-l border-gray-200 flex flex-col h-[40vh] md:h-full fixed bottom-[64px] md:static md:bottom-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] md:shadow-none z-20">
        <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-blue-600 text-white md:bg-white md:text-gray-800">
          <h2 className="font-bold text-lg flex items-center gap-2">
            <ShoppingCart className="w-5 h-5" /> Current Sale
          </h2>
          <span className="bg-white/20 md:bg-gray-100 px-3 py-1 rounded-full text-sm font-mono">
            {cart.length} items
          </span>
        </div>
            ...
            cart.map((item) => (
              <div key={item.id} className="flex items-center justify-between bg-gray-50 p-3 rounded-lg">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => updateCartQuantity(item.id, item.quantity - 1)} className="p-1 hover:bg-gray-200 rounded">
                  ...
                  <button onClick={() => removeFromCart(item.id)} className="p-1 hover:bg-red-100 rounded ml-2">
      ...
      {showReceipt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm shadow-2xl text-center animate-in fade-in zoom-in duration-300">
            <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4">
              <Receipt className="w-8 h-8" />
            </div>
            <h2 className="text-2xl font-bold text-gray-800 mb-2">Payment Success!</h2>
            <p className="text-gray-500 mb-6">Transaction recorded successfully.</p>
            <div className="border-t border-b border-dashed border-gray-300 py-4 mb-4">
               <p className="text-4xl font-bold text-gray-900">₱{cartTotal.toLocaleString()}</p>
            </div>
            <p className="text-sm text-gray-400">Printing receipt...</p>
          </div>
        </div>
      )}
    </div>
